  ...
  return (
    <div
      className={`w-64 shrink-0 snap-start rounded-2xl border bg-white p-4 shadow-sm ${
        recommended ? 'border-gold-400 ring-1 ring-gold-100' : 'border-sage-200'
      }`}
    >
      {recommended && (
        <span className="mb-2 inline-block rounded-full bg-gold-400 px-2.5 py-0.5 text-[11px] font-semibold text-ink-900">
          Recommended
        </span>
      )}
      ...
      <div className="mt-3 flex flex-wrap gap-1.5">
        <span className="rounded-full bg-sage-100 px-2 py-0.5 text-[11px] text-ink-700">
          Sleeps {room.max_guests}
        </span>
        {room.breakfast_included && (
          <span className="rounded-full bg-gold-50 px-2 py-0.5 text-[11px] text-gold-700">
            Breakfast included
          </span>
        )}
      </div>
